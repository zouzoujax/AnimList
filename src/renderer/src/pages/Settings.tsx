 ...
 */
function filterSettings(root: HTMLElement, query: string): Set<string> | null {
                ...
                >
                  {discord.showing ? (
                    ...
                    <span className="text-muted">
                      Rien en cours de lecture. Lance un épisode : cette ligne dira ce qui part sur ton profil. Si elle
                      se remplit et que Discord n’affiche toujours rien, c’est son réglage «&nbsp;Statut
                      d’activité&nbsp;» qui est en cause, pas l’app.
                    </span>
                  )}
                </div>
              )}
        ...
        <Card id="suivis" title="Ce que tu suis" icon={<BellRing size={17} />}>
          {follows.length === 0 ? (
            <p className="px-1 py-2 text-[0.82rem] leading-relaxed text-muted">
              Aucun suivi. Sur la page d’un studio ou d’un doubleur, « Suivre » te fera prévenir de ses prochaines
              sorties — et elles remonteront sur l’accueil.
            </p>
          ) : (
